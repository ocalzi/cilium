...
func (ops *envoyOps) Initializer(config CECConfig, params reconciler.Params) job.OneShotFunc {
	return func(ctx context.Context, _ cell.Health) error {
		// Wait for EnvoyResources table to be initialized.
		...
		// that Envoy can be started with an up-to-date snapshot. Don't wait for ACKs and don't
		// ACK proxy ports, that is done later once the reconciler picks up these resources.
...
func (ops *envoyOps) initializeResources(ctx context.Context, txn statedb.ReadTxn) error {
	...
	return ops.xds.InitializeEnvoyResources(ctx, merged)
}
...
func registerEnvoyReconciler(
	...
	fence.Add("ciliumenvoyconfig", func(ctx context.Context) error {
		...
		select {
		case <-ctx.Done():
			log.Warn("Failed waiting on CiliumEnvoyConfig reconciler initialization", logfields.Error, ctx.Err())
		case <-ops.initDone:
		}
		return nil
	})
